...
class FunctionalTest extends GMockTestCase {
    ...
    void setUp() {
        ...
        sql.execute("create table respository (key char(10) primary key, value char(10))")
        (1..5).each { i ->
            sql.execute("insert into respository(key, value) values (?, ?)", ["key $i", "value $i"])
        }
    }

    void tearDown() {
        sql.execute("drop table respository")
    }

    void testLRUCache() {
        Respository hr = new HSQLDBRespository()
        Strategy ls = new LRUStrategy()
        Cache cache = new Cache(hr, ls, 5)

        assertEquals "value 1", cache.get("key 1") // cache: [key 1: value 1]
        cache.put "key 1", "new value"             // cache: [key 1: new value]
        assertEquals "value 1", sql.firstRow("select value from respository where key = 'key 1'").value
        ...
        assertNull sql.firstRow("select * from respository where key = 'key 6'")
        ...
        assertEquals "new value", sql.firstRow("select value from respository where key = 'key 1'").value
        ...
        assertEquals "value 6", sql.firstRow("select value from respository where key = 'key 6'").value
        assertEquals "value 2", sql.firstRow("select value from respository where key = 'key 2'").value
        assertNull sql.firstRow("select * from respository where key = 'key 7'")

        cache.flush()
        assertEquals "new value", sql.firstRow("select value from respository where key = 'key 2'").value
        assertEquals "value 7", sql.firstRow("select value from respository where key = 'key 7'").value
    }
    ...
    void testRandomCache() {
        ...
        Respository hr = new HSQLDBRespository()
        Strategy rs = new RandomStrategy()
        Cache cache = new Cache(hr, rs, 5)
        ...
        play {
            ...
            assertEquals "value 3", sql.firstRow("select value from respository where key = 'key 3'").value
            ...
            assertEquals "new value", sql.firstRow("select value from respository where key = 'key 3'").value
